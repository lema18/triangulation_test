...
void search_existent_points(int ident,vector<int> ref_match_idx,cv::Mat &used_points,vector<int> &identifiers,my_slam obj)
{
    ...
    {   
        ...
        if(!stop_flag)
        {
            used_points.at<double>(i)=0;
            identifiers.push_back(0);
        }   
    }
}
void update_3d_point(my_slam &obj,int identifier,int match_id,cv::Point3d pt)
{
...
double calcMeanDepth(const vector<Point3d> &pts_3d)
{
    double mean_depth = 0;
    for (const Point3d &p : pts_3d)
        mean_depth += p.z;
    mean_depth /= pts_3d.size();
    return mean_depth;
}
void scalePointPos(Point3d &p, double scale)
{
    p.x *= scale;
    p.y *= scale;
    p.z *= scale;
...
void generate_projection_matrix(cv::Mat &projection_matrix,cv::Mat &motion_between_cams)
{
    cv::Mat inter=cv::Mat::zeros(3,4,CV_64F);
    inter.at<double>(0,0)=motion_between_cams.at<double>(0,0);
    inter.at<double>(0,1)=motion_between_cams.at<double>(0,1);
    inter.at<double>(0,2)=motion_between_cams.at<double>(0,2);
    inter.at<double>(1,0)=motion_between_cams.at<double>(1,0);
    inter.at<double>(1,1)=motion_between_cams.at<double>(1,1);
    inter.at<double>(1,2)=motion_between_cams.at<double>(1,2);
    inter.at<double>(2,0)=motion_between_cams.at<double>(2,0);
    inter.at<double>(2,1)=motion_between_cams.at<double>(2,1);
    inter.at<double>(2,2)=motion_between_cams.at<double>(2,2);
    inter.at<double>(0,3)=motion_between_cams.at<double>(0,3);
    inter.at<double>(1,3)=motion_between_cams.at<double>(1,3);
    inter.at<double>(2,3)=motion_between_cams.at<double>(2,3);
    projection_matrix=inter;
}
...
void generate_4x4_transformation(cv::Mat &R,cv::Mat &t,cv::Mat &Transformation)
{
    Transformation.at<double>(0,0)=R.at<double>(0,0);
    Transformation.at<double>(0,1)=R.at<double>(0,1);
    Transformation.at<double>(0,2)=R.at<double>(0,2);
    Transformation.at<double>(1,0)=R.at<double>(1,0);
    Transformation.at<double>(1,1)=R.at<double>(1,1);
    Transformation.at<double>(1,2)=R.at<double>(1,2);
    Transformation.at<double>(2,0)=R.at<double>(2,0);
    Transformation.at<double>(2,1)=R.at<double>(2,1);
    Transformation.at<double>(2,2)=R.at<double>(2,2);
    Transformation.at<double>(0,3)=t.at<double>(0);
    Transformation.at<double>(1,3)=t.at<double>(1);
    Transformation.at<double>(2,3)=t.at<double>(2);
}
                                            ...
                                            double scale,cv::Mat &mask,vector<Point3d> &pts3d,my_slam &obj)
{
    ...
    cv::Mat curr_rel_motion=cv::Mat::eye(4,4,CV_64F);
    generate_4x4_transformation(R,t,curr_rel_motion);
    cv::Mat new_camera_pose=last_pose*curr_rel_motion.inv();
    obj.cam_poses[current_frame]=new_camera_pose;
    cv::Mat left_project_mat=cv::Mat::zeros(3,4,CV_64F);
    cv::Mat right_project_mat=cv::Mat::zeros(3,4,CV_64F);
    cv::Mat last_rel_to_ref;
    cv::Mat curr_rel_to_ref;
    rel_motion_2_to_1(cv::Mat::eye(4,4,CV_64F),last_pose,last_rel_to_ref);
    rel_motion_2_to_1(cv::Mat::eye(4,4,CV_64F),new_camera_pose,curr_rel_to_ref);
    generate_projection_matrix(left_project_mat,last_rel_to_ref);
    generate_projection_matrix(right_project_mat,curr_rel_to_ref);
    cv::Mat point3d_homo;
    vector<Point2d> normalized_left,normalized_right;
    for(unsigned int i=0;i<triangulation_points_left.size();i++)
    {
        cv::Point2d left,right;
        pixel_to_cam_plane(triangulation_points_left[i],left,intrinsic);
        normalized_left.push_back(left);
        pixel_to_cam_plane(triangulation_points_right[i],right,intrinsic);
    ...
    double mean_depth = calcMeanDepth(pts3d);
    t /= mean_depth;
    for (Point3d &p : pts3d)
    ...
    //update map_of_points if they are new push to map other way do the average between the new and the existen point
    else
    {
        cv::Mat used_features=cv::Mat::zeros(1,(int)inliers_recover_left.size(),CV_8UC1);
        vector<int> identifiers;
        search_existent_points(obj.ident,inliers_recover_left,used_features,identifiers,obj);
        for(int i=0;i<used_features.rows;i++)
        {
            if(used_features.at<unsigned int>(i)==0)
            {
                add_new_3d_point(last_frame,current_frame,pts3d[i],inliers_recover_left[i],inliers_recover_right[i],obj);
            }
            else
            {
                update_3d_point(obj,identifiers[i],inliers_recover_right[i],pts3d[i]);
            }
