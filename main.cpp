...
int main(int argc,char ** argv)
{
    if(argc<10)
    {
        ...
        cout << "argv[10]=number of frames to skip\n";
        exit(-1);
    }
    ...
    //we need to skip a minimun of frames cuz other way triangulation will fail
    int count;
    sscanf(argv[10],"%d",&count);
	...
	for (unsigned int i = 0; i < frames_info.valid_frames.size(); i++)
    {
        if(frames_info.valid_frames[i]==1)
        {
            ...
            viewer.addCoordinateSystem(0.10, cam_pos, name);
    ...
    read_poses_from_ground_truth(argv[8],current_frame,camera_poses);
  ...
  /*  obj.imgs[last_frame]=loadImage(argv[1],last_frame,intrinsic,distcoef);
    obj.cam_poses[last_frame]=load_pose_from_groundtruth(argv[8],last_frame,nImages);
    obj.frames_id.push_back(last_frame);
    obj.valid_frames.push_back(1);
    calculate_features_and_descriptors(last_frame,obj);
    while(score>dbow2_threshold)
    {
        current_frame++;
        obj.imgs[current_frame]=loadImage(argv[1],current_frame,intrinsic,distcoef);
        calculate_features_and_descriptors(current_frame,obj);
        vector<cv::Mat> left_descriptors,right_descriptors;
        changeStructure(obj.descriptors[last_frame],left_descriptors);
        changeStructure(obj.descriptors[current_frame],right_descriptors);
        score=calculate_score(argv[9],last_frame,current_frame,left_descriptors,right_descriptors);
        if(score>dbow2_threshold)
        {
            obj.valid_frames.push_back(0);
    ...
    //valid frame detected lets match features
    vector<Point2f> left_points,right_points;
    vector<int> left_idx,right_idx;
    cv::Mat mask;
    cv::Mat E;
    matchFeatures_and_compute_essential(last_frame,current_frame,left_points,right_points,left_idx,right_idx,mask,dst_ratio,
    E,focal_length,confidence,reproject_err,principal_point,obj);
    double scale=1;
    vector<Point3d> triangulated_points;
    estimate_motion_and_calculate_3d_points(last_frame,current_frame,left_points,right_points,left_idx,right_idx,
    intrinsic,E,focal_length,principal_point,scale,mask,triangulated_points,obj);
    //visualization
    pcl::visualization::PCLVisualizer viewer("Viewer");
	viewer.setBackgroundColor(0.35, 0.35, 0.35); 
	...
	for (unsigned int i = 0; i < obj.valid_frames.size(); i++)
    {
        if(obj.valid_frames[i]==1)
        {
            ...
            cv::Mat cv_cam_rot;
            cv::Mat cv_cam_tras;
            cv_cam_rot=obj.cam_poses[i].rowRange(0,3).colRange(0,3);
            cv_cam_tras=obj.cam_poses[i].rowRange(0,3).col(3);
            cv2eigen(cv_cam_rot,cam_rotation);
            cv2eigen(cv_cam_tras,cam_translation);
            eig_cam_pos.block<3,3>(0,0)=cam_rotation;
            eig_cam_pos.block<3,1>(0,3) = cam_translation;
            ...
            file << obj.frames_id[i] << " " << cam_pos(0,3) << " " <<  cam_pos(1,3) << " " << cam_pos(2,3) << " " << 
                q.x() << " " << q.y() << " " << q.z() << " " << q.w() << std::endl;
        }
    }
    ...
    file.close();
	pcl::PointCloud<pcl::PointXYZ> cloud;
    for(unsigned int j=0;j<triangulated_points.size();j++)
    {
        pcl::PointXYZ p(triangulated_points[j].x, triangulated_points[j].y,triangulated_points[j].z);
        cloud.push_back(p);
    }
	viewer.addPointCloud<pcl::PointXYZ>(cloud.makeShared(), "map");
